...
class Network(nn.Module):
    def __init__(self):
        ...
        self.ff = nn.Sequential(
            nn.Linear(PLACEHOLDERINPUTFEAT, 64),
            nn.ReLU(),
            nn.Linear(64, 512),
            nn.ReLU(),
            nn.Linear(512, 512),
            nn.ReLU(),
            nn.Linear(512,PLACEHOLDEROUTPUTFEAT),
        )
...
def select_action(eps_threshold):
    ...
    if sample > eps_threshold:
        ...
        var = random.randint(0,PLACEHOLDERMINUS)
        #var = LongTensor([[random.randrange(3)]])
...
def run_single_actor(bogus, real, head, e, environment,agent,label,memory,state,list_of_actions):
       ...
       while done != 1:
            ...
            if bias > 5 and real != "real":
                #print ("LOG4 Tia bia passou dos tampos")
                bias = 0 
            ...
            print("BASE STATE ", state, next_state)
            state = next_state
            steps += 1
...
for e in range(EPISODES):
    head = 0
    state = env.reset()
    list_of_actions = (PLACEHOLDERFORPLACES)
    list_of_actions2 = (PLACEHOLDERFORPLACES)
    ...
    reward2 = [0] * PLACEHOLDERGATEWAYS
    reward = [0] * PLACEHOLDERGATEWAYS


    r = list(range(PLACEHOLDERGATEWAYS))
    random.shuffle(r)
    ...
    if ( episode_result >= keiser ):
        print(" LOG4 REPO saving the sum")
    ...
    if (episode_result > keiser ):
        ...
        file.close()
        print("REPORT LOG4 Saving the best status ",keiser)
    ...
    print("LOG4 ============================================================================================================================ SAR",)
    for i in range(PLACEHOLDERGATEWAYS):
