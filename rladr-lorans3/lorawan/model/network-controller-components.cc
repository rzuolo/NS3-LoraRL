  ...
  if (mHdr.GetMType () == LorawanMacHeader::CONFIRMED_DATA_UP)
    {
      ...
      // int SizeP[5] = {57,15,19,15,22};
	  //  int size = SizeP[(rand() % 5)];
	  int size = 12;
	
	 //  uint8_t resposta[size] = {0} ;
     //   uint8_t resposta[std::to_string(size).length()] = { uint8_t(SizeP[size]) };
	//int size = std::to_string(rule).length() ; 
    //status->m_reply.payload = Create <Packet> (resposta,size);
	 status->m_reply.payload = Create <Packet> (size);


	 NS_LOG_DEBUG ("Starting Header Removal at NetworkComponents OnReceivedPacket Class");
     Ptr<Packet> tempkt = packet->Copy();
     NS_LOG_DEBUG ("Reply of Packet: " << status->m_reply.payload);
     NS_LOG_DEBUG ("Original Packet: " << myPacket);
     ...
     tempkt->CopyData(&retrv,tempkt->GetSize());
	  ...
	  NS_LOG_DEBUG ("################Size as of NetworkComponents OnReceivedPacket Class #####################  "<< tempkt->GetSize() );
      NS_LOG_DEBUG ("################Content as of NetworkComponents OnReceivedPacket Class ####################  "<< str.str() );
