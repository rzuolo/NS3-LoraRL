      ...
      if (sender != (*i))
        {
          ...
          //**************************tests
          //std::cout<< " This is the sender... "<< sender->GetDevice()->GetNode()->GetId() << std::endl;
          //std::cout<< " This is the receiver  "<< dstNetDevice->GetNode ()->GetId () << std::endl;


          //uint8_t strg = uint8_t(sender->GetDevice()->GetNode()->GetId()+48);
          //std::cout << "Adding this to pkg ..."<< strg << std::endl;
          //Ptr <Packet> addpacket = Create<Packet> ( &strg, 2);
          //packet->AddAtEnd(addpacket);
          //**************************tests
        }
    }
       ...
       /*  NS_LOG_DEBUG ("Starting Header Removal at LoraChannel-Send Class");
        ...
        NS_LOG_DEBUG ("################Size as of LoraChannel-Send Class #####################  "<< tempkt->GetSize() );
        NS_LOG_DEBUG ("################Content as of LoraChannel-Send Class ####################  "<< str.str() );
                      ...
                      LoraChannelParameters parameters) const
{
  NS_LOG_FUNCTION (this << i << packet << parameters);
     ...
     /*
         ...
         NS_LOG_DEBUG ("Starting Header Removal at LoraChannel-Receive Class");
        Ptr<Packet> tempkt = packet->Copy();
        NS_LOG_DEBUG ("Copy of Packet: " << tempkt);
        NS_LOG_DEBUG ("Original Packet: " << packet);
        LorawanMacHeader macHdr;
        ...
        retrv.rdbuf(&str);
        tempkt->CopyData(&retrv,tempkt->GetSize());
        NS_LOG_DEBUG ("################Size as of LoraChannel-Receive Class #####################  "<< tempkt->GetSize() );
        NS_LOG_DEBUG ("################Content as of LoraChannel-Receive Class ####################  "<< str.str() );
        //ApplyNecessaryOptions (frameHdr);
        //tempkt->AddHeader (frameHdr);
        //ApplyNecessaryOptions (macHdr);
        //tempkt->AddHeader (macHdr);
