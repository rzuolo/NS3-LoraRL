...
EndDeviceLorawanMac::Send (Ptr<Packet> packet)
{
	...
	//**************************tests
    //std::cout<<"Counting01 crows"<<std::endl;
	//std::ostream retrv(nullptr);
        //std::stringbuf str;
        //retrv.rdbuf(&str);
        //packet->CopyData(&retrv,packet->GetSize());
        //NS_LOG_DEBUG ("#####?###########Size sent to Phy "<< packet->GetSize() );
        //NS_LOG_DEBUG ("#####?###########Content sent to Phy "<< str.str() );
	//**************************tests
  ...
  //std::cout<<"Counting02 crows"<<std::endl;
 ...
 // std::cout<<"Counting03 crows"<<std::endl;
  ...
  // the transmitting channel is available and we have not run out the maximum number of retransmissions
    {
	  //std::cout<<"Counting04 crows"<<std::endl;
...
EndDeviceLorawanMac::postponeTransmission (Time netxTxDelay, Ptr<Packet> packet)
{
  ...
  //**** tests
  //std::cout<< "Counting2.5 crows"<<std::endl;
  //****** tests
  ...
  if (packet != m_retxParams.packet)
    {
      ...
      if (mHdr.IsUplink())
    	  ...
    	  frameHdr.SetAdr(true);
      	  ...
      	  {
    	  std::cout<<"Won't do anything because it is a downlink"<<std::endl;
      	  }
      //****tests
      ...
      if (frameHeader.GetAck ())
        {
          ...
          //****** tests
          //bool test;
          //test = frameHeader.GetAdr();
          //std::cout<<"ADR status..."<< test << std::endl;
		  //****** tests
          uint8_t txs = m_maxNumbTx - (m_retxParams.retxLeft);
                                ...
                                std::list<int> enabledChannels, int repetitions)
{
  ...
  //*********tests
  //std::cout<<"here we are running a LinkAdrReq..."<< dataRate << std::endl;
  //*********tests
