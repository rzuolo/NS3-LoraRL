...
GatewayLorawanMac::Receive (Ptr<Packet const> packet)
{
  ...
  if (macHdr.IsUplink ())
  ...
  else
    {
     ...
     //**************************tests
     /*   NS_LOG_DEBUG ("Starting Header Removal at GatewayLoraMac Receive Class");
        ...
        NS_LOG_DEBUG ("################Size as of GatewayLoraMac Receive Class #####################  "<< tempkt->GetSize() );
        NS_LOG_DEBUG ("################Content as of GatewayLoraMac  Receive Class ####################  "<< str.str() );
	*/
