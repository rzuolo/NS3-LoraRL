                                  ...
                                  const Address& gwAddress)
{
  ...
  // tests****************************************
  //NS_LOG_FUNCTION (this << packet.payload);
  // tests****************************************
  ...
  std::stringbuf str;
  retrv.rdbuf(&str);
  myPacket->CopyData(&retrv,myPacket->GetSize());
  NS_LOG_DEBUG ("The NETWORK STATUS RECEIVED SIZE  "<< myPacket->GetSize() );
  NS_LOG_DEBUG ("The NETWORK STATUS RECEIVED CONTENT  "<< str.str() );
  //************************ tests
...
NetworkStatus::SendThroughGateway (Ptr<Packet> packet, Address gwAddress)
{
  ...
  m_gatewayStatuses.find (gwAddress)->second->GetNetDevice ()->Send (packet,
   ...
   NS_LOG_DEBUG ("The response size will be "<< packet->GetSize() );
   NS_LOG_DEBUG ("The response package will be "<< str.str() );
   //********************************tests
