...
LoraRadioEnergyModel::ChangeState (int newState)
{
  ...
  Ptr<Node> node = m_source->GetNode();

  //std::cout<<"someone is willing to know the energy consumed"<< m_totalEnergyConsumption << " "<< node->GetId() <<std::endl;

  ///Trying to calcultae the minimum power used over all transmissions
  if( energyToDecrease < min_diff_power_total_overt_all_ts )
  {
	  min_diff_power_total_overt_all_ts = energyToDecrease;
  }

  trialdevices[node->GetId()].pwr_i = trialdevices[node->GetId()].pwr_ii;
  trialdevices[node->GetId()].pwr_ii = energyToDecrease;
  trialdevices[node->GetId()].pwr_total = trialdevices[node->GetId()].pwr_total + energyToDecrease;

  global_power =  global_power + energyToDecrease;




  //std::cout<<"% energy spent in the last round "<<m_totalEnergyConsumption<<" "<<global_power <<" " << 100*(float(trialdevices[node->GetId()].pwr_ii))/float(global_power)<<std::endl;

  //std::cout<<"Energy consumed in previous cycle "<< trialdevices[node->GetId()].pwr_i <<std::endl;
  //std::cout<<"Spent in this cycle "<< trialdevices[node->GetId()].pwr_ii <<std::endl;
  //std::cout<<"Sum of energy spent "<< trialdevices[node->GetId()].pwr_total <<std::endl;
