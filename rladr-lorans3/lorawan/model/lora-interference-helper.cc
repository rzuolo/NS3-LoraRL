  ...
  for (it = m_events.begin (); it != m_events.end ();)
    {
      ...
      Ptr<LoraInterferenceHelper::Event> interferer = *it;
          ...
          //****************tests
      	  //std::cout<<"duration "<< interferer->GetDuration() <<"sf "<< int(interferer->GetSpreadingFactor()) << "rxpower "<<  interferer->GetRxPowerdBm() <<" frequency "<< interferer->GetFrequency() << std::endl;
          //*****************tests
      ...
      if (snir >= snirIsolation)
        {
          ...
          //**************tests
          //std::cout<<"Received SNIR "<< snir << " SF "<< currentSf << " Power "<< rxPowerDbm << " snirIsolation " << snirIsolation <<std::endl;
          //**************tests
      ...
      else
        {
          ...
          //**************tests
          //std::cout<<"Destroyed SNIR "<< snir << " SF "<< currentSf << "Power "<< rxPowerDbm << " snirIsolation " << snirIsolation <<std::endl;
          //**************tests
                                        ...
                                        Ptr<LoraInterferenceHelper::Event> event2)
{
  ...
  if (s1-s2 < 10 && s1-s2 >0 )
  {
	  std::cout<<"s1 "<< s1 << " difference " << s1-s2 <<" s2 " << s2<<std::endl;
  }
  //************  tests
